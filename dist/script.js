...
"use strict";
__webpack_require__.r(__webpack_exports__);
var burger = function burger() {// document.body.classList.toggle('_lock')
};
...
var spollerModule = function spollerModule() {
  var spollerArray = document.querySelectorAll('[data-spollers]'); //spoller wrapper (all spollers)

  if (spollerArray.length > 0) {
    //if exist
    //Разделям сполеры на обычные и по брек поинту (Получаем обичные)
    var spollersRegular = Array.from(spollerArray).filter(function (item, index, self) {
      return !item.dataset.spollers.split(",")[0]; //return all USSUAL spollers that not contains ',' (Array)
    });

    if (spollersRegular.length > 0) {
      //if ussual spollers exist
      initSpollers(spollersRegular); //Call Fun
    }

    var spollersMedia = Array.from(spollerArray).filter(function (item, index, self) {
      return item.dataset.spollers.split(",")[0]; //return all MEDIA spollers that contains ',' (Array)
    });

    if (spollersMedia.length > 0) {
      //if media spollers exist
      var breakpointsArray = []; //этот масив будет наполняться параметрами

      spollersMedia.forEach(function (item) {
        var params = item.dataset.spollers; //Строка с параметрами для каждого item (650,min) (800,max)

        var breakpoint = {};
        var paramsArray = params.split(","); //String to Array    now (["650", "min"])
      ...
      mediaQueries.forEach(function (breakpoint) {
        var paramsArray = breakpoint.split(","); //Розбиваем масив по ',' 0: '(min-width: 650px)' 1: '650' 2: 'min'

        var mediaBreakpoint = paramsArray[1]; // 650

        var mediaType = paramsArray[2]; // min

        var matchMedia = window.matchMedia(paramsArray[0]); // min-width: 650px
    ...
    function initSpollers(spollersArray) {
      ...
      spollersArray.forEach(function (spollersBlock) {
        ...
        if (matchMedia.matches || !matchMedia) {
          // matchMedia.matches брекПоинт сработал или matchMedia == false
    ...
    function initSpollerBody(spollersBlock) {
      ...
      var spollerTitles = spollersBlock.querySelectorAll('[data-spoller]'); //in what we click (btn) All
    ...
    function setSpollerAction(e) {
      ...
      if (el.hasAttribute('data-spoller') || el.closest('[data-spoller]')) {
        //якшо то шо ми клiкнули мае атрибук 
        var spollerTitle = el.hasAttribute('data-spoller') ? el : el.closest('[data-spoller]'); //btn

        var spollersBlock = spollerTitle.closest('[data-spollers]'); //block

        var oneSpoller = spollersBlock.hasAttribute('data-one-spoller') ? true : false; //если есть 'data-one-spoller' буде true

        if (!spollersBlock.querySelectorAll('_slide').length) {
          //Якшо нема '_slide' для коректной работы
          if (oneSpoller && !spollerTitle.classList.contains('_active')) {
            //oneSpoller == true  title нема '_active'
    ...
    function hideSpollerBody(spollersBody) {
      var spollerActiveTitle = spollersBody.querySelector('[data-spoller]._active');

      if (spollerActiveTitle) {
        //Якшо э активиний клас
